...
(define (instance-data-properties instance-data)
  ...
  ;; 在object定义之后调用了,可以用于实现pp 2025年2月27日19:55:04
...
(define (disable-debugging)
  (if debug-output
      (set! debug-output #f)))
;;; 
(define-syntax 定义链式广义过程
  (syntax-rules ()
    [(_ name arity default-handler)
     (define name
       (chaining-generic-procedure (quote name) arity default-handler))]))
...
(define-syntax 广义过程扩展
  (syntax-rules ()
    [(_ name super ((pred1 arg1) (pred2 arg2) ...) body ...)
     (define-generic-procedure-handler name
       (match-args pred1 pred2 ...)
       (lambda (super arg1 arg2 ...)
	 body ...))]
