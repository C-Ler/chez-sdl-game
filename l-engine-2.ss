  ...
  (let ((系统初始化标志 0))
    (letrec ((return (lambda (msg)
			 ...
			 [(mix-init)
			  ...
			  (mix-init (bitwise-ior MIX_INIT_FLAC MIX_INIT_MP3 MIX_INIT_OGG))
...
(define (mg-quit-sdl manager)
  (manager 'quit))
(define (mg-img-init manager)
  (manager 'image-init))
(define (mg-ttf-init manager)
  (manager 'ttf-init))
(define (mg-mix-init manager)
  (manager 'mix-init))
...
(define (game-mk 每帧时长 标题str 窗口x 窗口y 窗口宽 窗口高)
  ...
  (let ((manager (manager-mk)))
    ...
    (mg-mix-init manager)
    (mg-net-init manager)
    (let* ((win (win-mk 标题str 窗口x 窗口y 窗口宽 窗口高)))
      (lambda (msg)
	(case msg
	  [(run)
	   (lambda (foo)
